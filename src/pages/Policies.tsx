...
export default function Policies() {
  ...
  return (
    <div className="pt-40 pb-24 px-8 md:px-16 max-w-[1440px] mx-auto w-full bg-surface transition-colors duration-300">
      <header className="mb-32">
        ...
        <h1 className="text-5xl md:text-6xl font-serif text-on-surface">Store Policies</h1>
        ...
        {sections.map((section, idx) => (
          ...
          >
            <div className="md:col-span-4">
              <h2 className="text-2xl font-serif text-on-surface">{section.title}</h2>
            </div>
            <div className="md:col-span-8 flex flex-col gap-6">
              <h3 className="text-3xl font-serif text-on-surface">{section.subtitle}</h3>
              <p className="text-lg font-light text-on-surface-variant max-w-2xl leading-relaxed">
                {section.content}
              </p>
            </div>
            {idx < sections.length - 1 && (
              <div className="col-span-12 mt-16 border-t border-outline-variant/30"></div>
            )}
          </motion.section>
        ))}
      </div>
